...
export const Library: React.FC = () => {
    ...
    const handleCloudFileClick = async (fileId: string, fileName: string) => {
        ...
        try {
            ...
            navigate('/viewer');
            loadFile(blob, fileName).catch(console.error);
        } catch (e) {
    ...
    const handleBoxItemClick = async (item: LibraryItem) => {
        if (item.type === 'folder') {
            ...
            try {
                ...
                navigate('/viewer');
                loadFile(blob, item.name).catch(console.error);
            } catch (e) {
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            navigate('/viewer');
            loadFile(file, file.name).catch(console.error);
        }
    };
            ...
            <div className="library-container">
                ...
                {cacheUsageMB > 0 && (
                    <div className="cache-info">
                        <span>📦 {cacheUsageMB.toFixed(1)}MB / 500MB</span>
                        <button className="cache-clear-btn" onClick={handleClearCache}>Clear</button>
                    </div>
                )}
